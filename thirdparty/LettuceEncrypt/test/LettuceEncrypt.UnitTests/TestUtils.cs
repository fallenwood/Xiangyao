  ...
  public static X509Certificate2 CreateTestCert(string[] domainNames, DateTimeOffset? expires = null) {
    ...
    while (retries > 0) {
      try {
        ...
        var cert1 = X509CertificateLoader.LoadPkcs12(certWithKey, string.Empty, X509KeyStorageFlags.PersistKeySet | X509KeyStorageFlags.Exportable);
#else
        var cert1 = new X509Certificate2(certWithKey, "", X509KeyStorageFlags.PersistKeySet | X509KeyStorageFlags.Exportable);
#endif
